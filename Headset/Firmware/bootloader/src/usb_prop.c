...
/*CustomHID_GetReport_Feature function prototypes*/
...
{
  /* Update the serial number string descriptor with the unique ID */
  uuid_get((char *)CustomHID_StringSerial + 2, CUSTOMHID_SIZ_STRING_SERIAL, 1);
...
void CustomHID_Reset(void)
{
  ...
  SetEPTxCount(ENDP0, Device_Property.MaxPacketSize);  // TODO: from Tracker
  SetEPRxValid(ENDP0);
...
void CustomHID_Status_In(void)
{
  if (Request == SET_REPORT) {
    Request = 0;
    // This callback means the buffer has been filled, so parse it
    feature_report_parse();
  }
}
...
uint8_t *CustomHID_GetReport_Feature(uint16_t Length)
{
  if (Length == 0)
  {
    pInformation->Ctrl_Info.Usb_wLength = feature_report_length(pInformation->USBwValue0, Length);
    return NULL;
  }
  else
  {
    return feature_report_get_buf(pInformation->USBwValue0, pInformation->Ctrl_Info.Usb_wOffset);
  }
}
...
uint8_t *CustomHID_SetReport_Feature(uint16_t Length)
{
  ...
  {
    return feature_report_set_buf(pInformation->USBwValue0, pInformation->Ctrl_Info.Usb_wOffset);
  }
}
...
uint8_t *CustomHID_GetStringDescriptor(uint16_t Length)
{
  ...
  // TODO: report this to ST so that it is fixed in future USB lib revisions
  if (wValue0 >= 4)
  {
    return NULL;
  }
